...
MODEL_EXPORT_PATH = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "models", "immo_property_regressor_model.pkl"))

COLS_TO_DROP = [
    "postal_code", "property_id", "posting_date", "property_type", "transaction_type",
    "new_construction", "price_per_sqm", "cadastral_income", "street", "locality", 
...
def create_model_pipeline(preprocessor: ColumnTransformer) -> Pipeline:
    """ preprocessor data transformer to a LinearRegression model engine."""
    pipeline = Pipeline(
        steps=[
...
def evaluate_and_diagnose(pipeline: Pipeline, X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, y_test: pd.Series) -> dict:
    """Calculates all metrics, prints them, and returns them inside a structured dictionary."""
    ...
    if r2_drop > 0.05:
        ...
        print(f"Is there overfitting? NO")
        
    # Return formatted metrics to main.py
    return {
...
def train_and_evaluate() -> dict:
    """Wrapper function to let main.py call this script directly and grab the metric results."""
    # clean & split data
    ...
    # model evaluation (now returns metrics dict)
...
if __name__ == "__main__":
    # If run on its own, execute the pipeline workflow normally
    train_and_evaluate()
